...
- **Movimientos:** Entrada Directa, Salida Directa (ambas con motivo: factura, ajuste, traspaso, devolución, merma, etc.), Inventario físico. Los traspasos son un **proceso de 2 pasos con confirmación**: la salida deja stock "en tránsito" hasta que el almacén destino confirma la entrada. **Toda operación es un documento con folio** (`ENT`, `SAL`, `TRA`, `REC`, `INV`), se carga a mano o por Excel, se revisa en una **vista previa que muestra el stock resultante antes de confirmar**, y se baja en PDF firmable.
...
1. Módulo `inventory` con **2 movimientos directos** + procesos especiales, todos escribiendo por **un único servicio** (`StockLedgerService.apply`): agrupa líneas, bloquea `stock_by_warehouse` con `SELECT … FOR UPDATE` **ordenado por (product_id, warehouse_id)** (anti-deadlock), valida, inserta en `stock_movements` y actualiza saldos en la misma transacción. Entradas, salidas, recepción de traspaso, conteo y (F4) la venta son **llamadores**, nunca escritores propios.
   ...
   - **Documentos, folios y PDF (F3-DOC, decisión de Carlos 2026-08-18)**: `inventory_documents` es el **encabezado** de toda operación que toca stock (folio, tipo, almacén, motivo, referencia, autorizador, quién y cuándo) y `stock_movements.document_id` la línea — la tabla nació con un `batch_id` implícito y F3-DOC-01 lo reemplaza. **Un prefijo por tipo de operación, 5 series por tenant**: `ENT` entrada directa, `SAL` salida directa, `TRA` traspaso (despacho), `REC` recepción, `INV` inventario físico; `VTA` reservada para F4. El motivo viaja dentro del documento, no en el folio. La serie vive en `tenant_sequences` con `INSERT … ON CONFLICT DO UPDATE … RETURNING` **dentro de la tx del ledger**: sin huecos (si la tx falla el número vuelve) a cambio de serializar por `(tenant, key)` mientras dure — barato en inventario, **a medir antes de reusarlo para el folio de ventas de F4**. El folio se toma **al confirmar, nunca en la vista previa**. `inventory_documents` es append-only igual que los movimientos: sin `updated_at`, con `REVOKE UPDATE, DELETE` y **todas** las FK en `RESTRICT` (en append-only un `ON DELETE SET NULL` es un UPDATE silencioso de la historia).
   - **Vista previa antes de escribir**: `POST /inventory/{entries,exits}/preview` es **stateless** y acepta **o** `lines[]` **o** un archivo (`file` + `format`), devolviendo la misma forma en los dos casos — por eso la pantalla es una sola cargues como cargues. Corre las mismas validaciones que el commit pero no escribe nada ni pide folio, y devuelve **`stockBefore`/`stockAfter` por línea**, los lotes nuevos, la expansión de compuestos y el reparto FEFO que se aplicaría. Sin tabla de borradores: mismo patrón que `counts/reconcile`.
...
6. Frontend: 2 pantallas de movimientos (cabecera reactiva por motivo, selector de presentación por línea, disponible en vivo en salida, UX teclado) + vista "Traspasos en tránsito" (dos tabs por scope, modal de recepción, badge > 7 días) + inventario físico en 2 pasos + tabs **Kardex** (con `balanceAfter` server-side) y **Stock por almacén** en el detalle de producto + selector de almacén reusable + UI de alcance por almacén en usuarios (deuda F2-SCOPE-03) + **Documentos de inventario** (listado buscable por folio, detalle y descarga del PDF). Entradas y salidas son de **dos pasos**: capturar (a mano o por Excel) → vista previa con el stock resultante → confirmar.
...
| **Documento de inventario** | El **encabezado** de una operación que toca stock (`inventory_documents`): folio, tipo, almacén, motivo, referencia, quién y cuándo. Sus líneas son los `stock_movements` que comparten `document_id`. Es lo que se lista, se busca por folio y se imprime en PDF. Append-only: no se edita ni se borra — se corrige con otro movimiento. |
| **Folio** | El número humano de un documento, `PREFIJO-000001`, con **una serie por tipo de operación y por tenant**: `ENT` entrada directa, `SAL` salida directa, `TRA` traspaso, `REC` recepción, `INV` inventario físico (`VTA` reservado para las ventas de F4). Se asigna **al confirmar**, dentro de la transacción: nunca en la vista previa. Sin huecos, a cambio de serializar las operaciones del mismo tipo del mismo tenant. |
| **Vista previa** | Paso obligatorio antes de escribir: el sistema resuelve las líneas (cargadas a mano o por Excel), las valida igual que el commit y devuelve **el stock actual y el resultante por línea** sin tocar nada ni consumir folio. Es lo que evita confirmar a ciegas una entrada de 80 productos. |
...
| **Traspaso** | Proceso de 2 pasos entre dos almacenes del mismo tenant: (1) Salida Directa motivo traspaso → crea `Transfer` con estado `in_transit` cuyo documento de despacho lleva folio `TRA-000001` (el traspaso no tiene folio propio: un folio, una fuente); (2) recepción en el destino (desde la vista de tránsito, todas las líneas, `0 ≤ recibido ≤ enviado`) → `Transfer` pasa a `completed` y nace su documento de recepción `REC-000001`, ligado al mismo traspaso. La discrepancia se deriva de las líneas y queda auditada. **Cancelar (solo TenantAdmin) no devuelve stock al origen**: el reingreso es un `adjustment` explícito. |
