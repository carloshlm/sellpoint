...
Una compra es **el papel del proveedor**, no un movimiento de stock. Guarda lo que dice la factura —fechas, folio del proveedor, líneas con costo, descuentos, impuestos y cargos— y no toca el inventario: la mercancía entra por una **entrada de inventario** que nace de ella. Esa asimetría es la ley del módulo: la compra **transporta** lo que se sabe (producto, presentación, cantidad, costo, lote, caducidad) y la entrada **exige** lo que el kardex necesita (lote obligatorio si el producto lo controla, ubicación, cantidades en presentaciones enteras). Por eso la compra acepta una línea sin lote y la entrada no: ahí es donde se completa, antes de confirmar. El transporte tiene un límite en la otra dirección: un lote en un producto que **no** se controla por lote no es algo que la entrada vaya a exigir sino algo que va a RECHAZAR, así que las líneas lo rebotan al capturar (422 `purchases.lot_not_tracked`, nombrando la línea) y el puente no copia el lote de un producto al que le apagaron el control después de confirmar (Carlos, 2026-09-11).

**El costo que cruza el puente es el NETO.** Confirmar una entrada por «Factura de compra» pisa `product_presentations.cost` y alimenta el costo promedio, así que si viajara el bruto con IVA el margen quedaría inflado 16 % para siempre. Al confirmar la compra se materializa `purchase_lines.unit_cost_net = (line_total − tax_amount) / quantity` (HALF_UP a 2) y es ESE número el que copia el puente — un contrato de punta a punta lo fija con una caja ×12 en los dos modos fiscales. El modo es **por compra** con default `excluded`, al revés del mostrador: la factura del proveedor viene neta más IVA.
