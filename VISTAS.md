...
| **El lector dispara aunque el cursor esté en otro campo** | Al escanear, el foco va al precio; si en vez de teclearlo se pasa la pistola por el siguiente producto, ese código entraba EN EL PRECIO. Se escucha el teclado del documento: si todas las teclas llegaron sin pausas de persona (50 ms) y lo que escribieron es un código válido, el campo recupera lo que tenía y el código entra por donde debía. Se DESHACE en vez de impedir: para saber que es un lector hacen falta varias teclas, y para entonces las primeras ya aterrizaron |
