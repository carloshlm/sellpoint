...
| El campo del código es un **formulario**, y ahí está el arreglo | En Android, con el teclado activo, el navegador reporta las teclas de hardware como `keyCode 229` / `key: "Unidentified"` — el 229 del IME. Un `onKeyDown` que compara contra «Enter» nunca se entera, y no se arregla comparando mejor. El envío del formulario sí se dispara en todos lados. El `onKeyDown` se queda para el escritorio y corta el envío, así que un Enter nunca agrega dos líneas |
| El código también se agrega con un **botón**, sin Enter | En un teclado de teléfono la tecla de retorno a veces es «Listo», a veces cierra el teclado y a veces no está: quien teclea el código a mano se quedaba sin forma de agregarlo. El campo además pide teclado numérico (`inputMode`) |
| El ejemplo del nombre y un nombre bloqueado **se ven distintos** | Se veían iguales, los dos en gris. Bloqueado: fondo gris y texto NEGRO, porque es contenido de verdad y lo único que no se puede es editarlo. Vacío: fondo blanco y el ejemplo en un gris claramente más tenue |
| **El lector dispara aunque el cursor esté en otro campo** | Al escanear, el foco va al precio; si en vez de teclearlo se pasa la pistola por el siguiente producto, ese código entraba EN EL PRECIO. Se escucha el teclado del documento: si lo escrito es un código válido **y** llegó a velocidad de máquina, el campo recupera lo que tenía y el código entra por donde debía. Se DESHACE en vez de impedir: para saber que es un lector hacen falta varias teclas, y para entonces las primeras ya aterrizaron |
| La velocidad se mide con el reloj del EVENTO y en PROMEDIO | Dos correcciones del mismo día. `Date.now()` se lee cuando el manejador alcanza a correr, y entre tecla y tecla React repinta la tabla: ese repintado se sumaba y un lector parecía una persona. `timeStamp` lo estampa el navegador cuando nace la tecla. Y se mide el promedio de la ráfaga entera, no cada par: UN hipo partía la ráfaga en dos y el búfer llegaba al Enter con una sola tecla dentro |
...
| La cámara solo se ofrece con el **dedo** como puntero | `(pointer: coarse)`, no el ancho de la ventana: una laptop con la ventana angosta sigue siendo una laptop, y su cámara apunta a la cara. **La regla vive dentro de `BarcodeScanner`** desde que el mostrador la pidió igual (Carlos, 2026-09-17): las dos pantallas que lo usan la quieren idéntica, y la tercera que venga también |
