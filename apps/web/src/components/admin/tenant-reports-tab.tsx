...
/**
 * F9-ADMIN-11 — los reportes de ventas, inventario y cierres de turno
 * (F5-SHIFT-05) del negocio, los MISMOS componentes que el cliente usa,
 * apuntados por el alcance del expediente.
 */
export function TenantReportsTab() {
  const { t } = useTranslation();
  const [vista, setVista] = useState<"sales" | "stock" | "shifts">("sales");
  ...
  return (
    <div className="flex flex-col gap-4" data-testid="tenant-reports">
      <div className="flex gap-2" role="tablist" aria-label={k("title")}>
        {(["sales", "stock", "shifts"] as const).map((v) => (
          <Button
            key={v}
            type="button"
            role="tab"
