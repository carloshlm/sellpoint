...
describe("Historia clínica — tablero", () => {
  it("el encabezado trae nombre, folio, edad, médico; sin sexo ofrece completar Datos Generales; 1 de 19", async () => {
  ...
  it("paciente M sin AGO: la tarjeta no se dibuja y el total baja a 18; con AGO capturado, se sigue viendo", async () => {
    ...
    expect(screen.getByTestId("record-header")).toHaveTextContent("0 de 18 secciones capturadas");
  });
  ...
  it("paciente M CON AGO capturado la sigue viendo: esconder no es borrar", async () => {
    ...
    expect(screen.getByTestId("record-header")).toHaveTextContent("1 de 19 secciones capturadas");
  });
