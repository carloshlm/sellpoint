...
describe("UsersAdminService.create (F1-RBAC-03)", () => {
  ...
  it("roleId de otro tenant (o inexistente) -> 400 users.invalid_role_ids, no crea nada", async () => {
    ...
    await expect(
      service.create(
        ACTOR,
        { email: "x@example.com", firstName: "X", lastName: "Y", roleIds: ["ajeno"] },
        {},
      ),
  ...
  it("email ya existente (unique GLOBAL, f1-auth D1) -> 409 users.email_taken", async () => {
    ...
    await expect(
      service.create(
        ACTOR,
        { email: "dup@example.com", firstName: "X", lastName: "Y", roleIds: ["role-1"] },
        {},
      ),
...
describe("UsersAdminService.create — W1b hardening (verify #274 pasada 2): escalada por asignación de roles", () => {
  it("ACTOR (SOLO users:manage, SIN roles:manage) no puede crear un user con un rol Admin -> 403, no crea nada", async () => {
    ...
    await expect(
      service.create(
        ACTOR,
        { email: "x@example.com", firstName: "X", lastName: "Y", roleIds: ["role-admin"] },
        {},
      ),
  ...
  it("ACTOR SÍ puede crear un user con un rol cuyos permisos posee por completo", async () => {
    ...
    await expect(
      service.create(
        ACTOR,
        { email: "y@example.com", firstName: "X", lastName: "Y", roleIds: ["role-1"] },
        {},
      ),
