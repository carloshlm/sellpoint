...
- [ ] **F3-HOME-01** — `users.default_warehouse_id` + validaciones + coherencia con el alcance
...
- [ ] **F3-HOME-02** — UI: selector «Almacén asignado» en `UserForm`
...
- [ ] **F3-HOME-03** — El tenant nace con almacén: `provision()` + onboarding + seed
...
- [ ] **F3-HOME-04** — Preselección operativa en movimientos
...
- [ ] **F3-HOME-05** — Sincronía de docs + coherencia F4
...
- **F4-DB** — Modelos `Sale`, `SaleItem`, `CashboxSession`. **`SaleItem` vende productos O servicios** (F3-SVC): `product_id` **nullable** + `service_id` nullable + CHECK «exactamente uno de los dos» (patrón de los índices parciales de F3); `presentation_id` es nullable y solo aplica al producto; `quantity` DECIMAL, `unit_price`, `discount`. **Reservar columnas nullable `Sale.clinical_document_id` y `Sale.quote_id`** para vincular ventas a documentos clínicos o cotizaciones cuando llegue Fase 9 (NO crear las tablas ahora, solo las FK nullable).
