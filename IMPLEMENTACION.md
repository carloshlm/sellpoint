...
- **2026-08-17 (DEFER.1, listo y EN ESPERA hasta el 22)** — **El fallback `?token=` está retirado en la rama `defer-1-retirar-fallback-token`; NO se mergea antes del 2026-08-22.** La fecha no es una preferencia, es una cuenta: el deploy de D3 (`#token=` en el fragmento) fue el **2026-08-15** y el TTL más largo de un link de mail es el de la **invitación, 7 días** → el último link con formato viejo muere el **22**. La verificación de email (24 h) y el reset de password (30 min) vencieron el 16 y el mismo 15. Retirarlo antes deja a quien tenga una invitación pendiente frente a una pantalla que no hace nada. Carlos eligió esperar; el trabajo queda hecho para que el 22 sea un merge y no una tarea. **Lo que cambia:** `readTokenFromUrl` lee SOLO el hash y un `?token=` se **ignora** —aceptarlo "por las dudas" mantendría viva la vía que D3 vino a cerrar—; el helper de los e2e (`extract-token-from-link.ts`) se recorta igual, porque es su espejo: si tolerara la query, un mail que volviera a emitirla pasaría los e2e y rompería en el navegador. Los 10 casos de `auth-flows.test.tsx` que todavía navegaban con `?token=` se migraron al formato vigente. **Verificación:** 239 e2e verdes con el helper aceptando solo `#token=` — eso prueba que el backend ya no emite el formato viejo por ninguna de las tres vías. — `topic_key: sellpoint/defer-1-token-query` — cierra (al mergear): DEFER.1
