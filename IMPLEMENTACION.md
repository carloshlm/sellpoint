...
| **Folio: lo lleva TODO movimiento** (Carlos, 2026-08-18) | **TRES series por tenant**: `ENT` Entrada Directa · `SAL` Salida Directa · `INV` Inventario físico. `VTA` **reservada para F4**. El motivo (factura, merma, ajuste, **traspaso**) viaja **dentro** del documento, no en el folio. **Corrección del mismo día, a pedido de Carlos:** la primera versión tenía 5 series con `TRA` y `REC` propias — y eso **contradecía la regla**, porque un traspaso *es* una Salida Directa con `reason_code='transfer'` y su recepción *es* una Entrada Directa con el mismo motivo (así está el modelo desde CU-MOV-01/03). Un traspaso es `SAL-000019` y su recepción `ENT-000043`; se encuentran filtrando por motivo. Formato `${prefix}-${n.padStart(6,'0')}`, crece más allá de 6 dígitos sin romper |
...
- [ ] **F3-DOC-01** — Tablas `inventory_documents` e `inventory_document_lines`
...
### Módulo F3-ENTRY — Entrada Directa
...
### Módulo F3-EXIT — Salida Directa (incluye el despacho de traspaso)
...
> El traspaso nace en F3-EXIT-01 (despacho), que es una **Salida Directa con motivo traspaso** — no un tipo de documento propio (corrección de Carlos, 2026-08-18). Acá vive el resto: listar según scope, ver detalle, **confirmar recepción** (que es un borrador de **Entrada Directa** con motivo traspaso, precargado), cancelar. El traspaso no tiene folio propio: el suyo es el de su documento de despacho. Recibido > enviado bloqueado; el cancelado no devuelve stock.
...
- [ ] Un **traspaso** completo: despacho como Salida Directa con motivo Traspaso (folio `SAL-000019`, origen baja, `in_transit`) → visible en "Pendientes de recibir" del destino y "Pendientes de enviar" del origen → «Confirmar recepción» abre un borrador de Entrada Directa precargado (`ENT-000043`) → se corrige el faltante con su nota → `completed`, destino sube lo recibido, diferencia derivada y auditada; recibido > enviado bloqueado; cancelación solo TenantAdmin con justificación y **sin** devolver stock; badge > 7 días
