...
- [ ] **F9-CLINIC-22** — `GET/PUT /medical-clinic/settings`
  - **Salida:** `settings.service.ts` (`get` devuelve la fila o los defaults `{sellsMedications: true, sellsLabStudies: true, sellsDiagnosticStudies: true}` sin crearla; `update` hace upsert con `updated_by` + auditoría `medical_clinic.settings.update` con before/after) y `medical-clinic-settings.controller.ts` (`@RequiresModule("medical_clinic")`, `@RequirePermissions("tenants:manage")`); `dto/settings.dto.ts` (tres booleanos, al menos uno presente; 422 `medical_clinic.empty_update`).
  - **Verificar:** unit spec (RED): sin fila → defaults en `true`; `PUT {sellsLabStudies:false}` crea la fila con los otros dos en `true`; segundo PUT actualiza sin duplicar; un Manager sin `tenants:manage` → 403; con el módulo apagado → 402.
  - **Depende de:** F9-CLINIC-21 · **Estimación:** 2 h

- [ ] **F9-CLINIC-23** — La orden respeta la configuración: cotización solo si se vende
  - **Salida:** en `medical-orders.service.ts#create`: lee `medical_clinic_settings` dentro de la tx (defaults si no hay fila) y decide por `kind`; con venta → camino de F9-CLINIC-14 (folio `COT-`, `quote_id`); sin venta → `nextFolio(tx, tenantId, "medical_order", "ORM")`, `quote_id: null`, las líneas de receta se resuelven igual (producto, presentación, descripción, precio de referencia) pero SIN tocar la cotización ni exigir stock (una receta que no se surte aquí no depende del almacén); respuesta `{id, folio, quoteId: string|null, chargeStatus}`; `#list` y `#cancel` según la decisión 30.
  - **Verificar:** unit spec (RED): `sells_lab_studies=false` → orden `ORM-000001` sin cotización y `chargeStatus:"not_for_sale"`; `true` → `COT-…` con cotización; receta sin `sells_medications` y producto sin stock → 201 (no exige stock); cancelar la `ORM-` solo cancela la orden. Mutante: ignorar la configuración (siempre cotizar) rompe el test. e2e en `medical-clinic-orders-pos.e2e-spec.ts`: la caja NO encuentra `ORM-…` en el buscador.
  - **Depende de:** F9-CLINIC-14, F9-CLINIC-22 · **Estimación:** 3 h

- [ ] **F9-CLINIC-24** — El documento carta de la orden
