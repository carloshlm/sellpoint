...
**Frontend:** `routes/` + `components/inventory` + `lib/inventory` (la convención `features/` se abandonó en F2). Rutas nuevas: `/movements/entries/new`, `/movements/exits/new`, `/movements/transfers`, `/movements/inventory-count`, `/movements/documents`; tabs **Kardex** y **Stock por almacén** dentro del detalle de producto (`/catalog/products`).
...
| **Folio: lo lleva TODO movimiento** (Carlos, 2026-08-18) | **Un prefijo por TIPO DE OPERACIÓN, 5 series por tenant**: `ENT` entrada directa · `SAL` salida directa · `TRA` traspaso (despacho) · `REC` recepción de traspaso · `INV` inventario físico. `VTA` **reservado para F4**. El motivo (factura, merma, ajuste…) viaja **dentro** del documento, no en el folio: dos entradas por motivos distintos son el mismo tipo de papel. Se descartó una serie por motivo (11 series que explicar) y una serie única `MOV` (dos documentos distintos se verían iguales en un listado). Formato `${prefix}-${n.padStart(6,'0')}`, crece más allá de 6 dígitos sin romper |
| Secuencia del folio | Tabla `tenant_sequences (tenant_id, key, next_value)` con `INSERT … ON CONFLICT DO UPDATE … RETURNING` **dentro de la transacción del ledger**; único por `(tenant_id, folio)`. Se descartó `MAX+1` (lock de tabla) y `SEQUENCE` global (revela volumen entre tenants). **Sin huecos, y eso cuesta serialización**: si la tx falla el número vuelve, pero el lock de esa fila serializa todas las operaciones del mismo tipo del mismo tenant mientras dure. Para inventario es correcto y barato. **Nota con nombre para F4**: en el POS, serializar cada venta sobre una fila puede doler en hora pico — medir antes de reusar tal cual |
| El folio se asigna al **confirmar**, nunca en la previa | Una vista previa que quema números deja huecos en la serie cada vez que alguien mira y se arrepiente. Es la regla que hace que mirar sea gratis |
...
| Documento append-only | `inventory_documents` sigue la misma regla que los movimientos: **sin `updated_at`**, con `REVOKE UPDATE, DELETE`, y **todas** las FK en `RESTRICT` — incluidas las opcionales (regla nacida en F3-DB-01: en una tabla append-only un `ON DELETE SET NULL` es un UPDATE silencioso de la historia) |
| Vista previa (Carlos, 2026-08-18) | **Stateless y con dos formas de carga en un solo endpoint**: `POST /inventory/{entries,exits}/preview` acepta **o** `lines[]` (lo que armó la tabla a mano) **o** `file` + `format` (el Excel), y devuelve **la misma forma** en los dos casos — por eso la pantalla es una sola. No escribe nada, no pide folio. Devuelve **`stockBefore` y `stockAfter` por línea** (el corazón del pedido: ver la suma al stock antes de que ocurra), `newLot`, la expansión de compuestos, el reparto FEFO que se aplicaría, `summary` y `errors[]` por fila con `row`/`field`. Sin tabla de borradores ni trabajo de limpieza — mismo patrón que `counts/reconcile`, que ya es un dry-run puro |
| PDF del documento | Renderizado **en el servidor** con **`pdfmake`**: la tabla tiene que **paginar sola con encabezado repetido** (un inventario físico son 500 líneas) y `pdfmake` lo hace declarativo donde `pdfkit` exigiría paginar a mano; server y no browser para que el papel salga idéntico venga de donde venga y para que F4/F5 puedan mandarlo por mail sin reescribir el layout. Contenido (Carlos): encabezado con `legalName` + `taxId` del tenant, folio, tipo, almacén, fecha, quién registró, motivo, referencia y nota; tabla de líneas; pie con firmas **Entregó / Recibió / Autorizó**. **Sin logo** (exige almacenamiento de archivos, fuera de F3). **Sin total de unidades**: sumar 36 unidades + 2.5 kg + 400 ml da un número que no significa nada — el pie muestra total de LÍNEAS y la cantidad va por línea con su unidad. Encabezado y pie comunes, **cuerpo por tipo** (una entrada muestra presentación y costo; un conteo muestra teórico/contado/diferencia) |
...
- [ ] **F3-DB-03** — Tabla `tenant_sequences` + `nextFolio()`
  - **Salida:** modelo `TenantSequence` → `tenant_sequences` (tenant_id, key VARCHAR(32), next_value BIGINT default 0; PK `(tenant_id, key)`); helper `nextFolio(tx, tenantId, key, prefix)` en `apps/api/src/modules/inventory/folio.ts` que ejecuta `INSERT … VALUES (tenant, key, 1) ON CONFLICT (tenant_id, key) DO UPDATE SET next_value = tenant_sequences.next_value + 1 RETURNING next_value` y formatea `${prefix}-${n.padStart(6,'0')}` (crece más allá de 6 dígitos sin romper; huecos por rollback aceptados y documentados); las **5 series** vienen de `FOLIO_PREFIXES` en `packages/shared` (F3-DOC-02): `ENT`, `SAL`, `TRA`, `REC`, `INV`, con `VTA` reservada para F4. **Docblock obligatorio con el gotcha del lock**: el `ON CONFLICT DO UPDATE` toma la fila `(tenant, key)` y no la suelta hasta el COMMIT, así que la serie no tiene huecos pero **serializa** todas las operaciones del mismo tipo del mismo tenant mientras dure la tx — correcto y barato en inventario, a medir antes de reusarlo tal cual para el folio de ventas de F4
  - **Verificar:** integración: 20 llamadas concurrentes en transacciones separadas producen 20 folios distintos y consecutivos; dos tenants arrancan ambos en `ENT-000001`; series distintas del mismo tenant no se pisan (`ENT-000001` y `SAL-000001` conviven); una tx que hace rollback **no** deja hueco en la serie
  - **Depende de:** —
  - **Estimación:** 1 h

- [ ] **F3-DB-04** — RLS + append-only en las 4 tablas nuevas
  - **Salida:** migración SQL: ENABLE + **FORCE** ROW LEVEL SECURITY + policy `tenant_isolation` (NULLIF) en `stock_movements`, `inventory_documents`, `transfers`, `transfer_lines`, `tenant_sequences`, `product_lots`, `stock_lots`; `REVOKE UPDATE, DELETE ON stock_movements, inventory_documents FROM sellpoint_app` (guardado con `IF EXISTS pg_roles`, patrón `units`; llega DESPUÉS del `ALTER DEFAULT PRIVILEGES` de F1, por eso hay que revocar explícito); `f3-rls.integration.spec.ts` con `RLS_TABLES = ['stock_movements','inventory_documents','transfers','transfer_lines','tenant_sequences','product_lots','stock_lots']` y los 4 canarios por tabla (propio ve / ajeno 0 / sin `set_config` 0 / WITH CHECK rechaza) + guardián estructural de FORCE
  - **Verificar:** la suite de integración pasa; UPDATE y DELETE sobre `stock_movements` **e `inventory_documents`** como `sellpoint_app` fallan con 42501; INSERT funciona; `tenant_sequences` sí admite UPDATE
  - **Depende de:** F3-DB-01, F3-DB-02, F3-DB-03, F3-DB-06, F3-DB-07, F3-DOC-01
  - **Estimación:** 2.5 h
...
### Módulo F3-DOC — Documentos, Folios y PDF

> **Toda operación que toca stock es un documento con folio.** Nace de la revisión de Carlos del 2026-08-18: un movimiento identificado por un UUID no se puede dictar por teléfono, anotar en una libreta ni buscar. Acá viven el encabezado, las 5 series de folio, el listado buscable y el PDF firmable. El folio se asigna **al confirmar**, nunca en la vista previa.

- [ ] **F3-DOC-01** — Tabla `inventory_documents` + `stock_movements.document_id`
  - **Salida:** enum Prisma `InventoryDocumentType { entry, exit, transfer_dispatch, transfer_receipt, physical_count }`; modelo `InventoryDocument` → `inventory_documents`: id, tenant_id, `folio VARCHAR(20)`, type, warehouse_id FK **RESTRICT**, linked_warehouse_id UUID NULL FK RESTRICT, reason_code `MovementReason` NOT NULL, reference VARCHAR(120) NULL, reason_note TEXT NULL, authorized_by UUID NULL FK users RESTRICT, transfer_id UUID NULL (FK en F3-DB-02), line_count INT NOT NULL, created_by FK users RESTRICT, created_at timestamptz `@default(dbgenerated("transaction_timestamp()"))`; **sin `updated_at`** y **todas** las FK en `RESTRICT`, incluidas las opcionales (regla de F3-DB-01: en append-only un `SET NULL` es un UPDATE silencioso de la historia); `@@unique([tenantId, folio])`; CHECK `line_count > 0`; índices `(tenant_id, type, created_at DESC)`, `(tenant_id, warehouse_id, created_at DESC)`, `(tenant_id, created_by)`. Migración: `ALTER TABLE stock_movements DROP COLUMN batch_id`, `ADD COLUMN document_id UUID NOT NULL` FK RESTRICT, índice `(tenant_id, batch_id)` → `(document_id)` — limpia porque la tabla está **vacía** en producción (`SELECT count(*)` verificado antes de escribir la migración)
  - **Verificar:** `inventory-schema.integration.spec.ts` crece: folio repetido en el mismo tenant rechazado y permitido en otro; borrar el almacén o el usuario de un documento falla (RESTRICT); no existe columna `updated_at`; `line_count = 0` rechazado; un movimiento sin `document_id` no se puede insertar; `prisma migrate diff` sin drift (ritual de F3-DB, nacido en F3-DB-01)
  - **Depende de:** F3-DB-01
  - **Estimación:** 2 h

- [ ] **F3-DOC-02** — `FOLIO_PREFIXES` + `DocumentsService.create()`
  - **Salida:** en `packages/shared` `FOLIO_PREFIXES: Record<InventoryDocumentType, string>` = `{ entry: 'ENT', exit: 'SAL', transfer_dispatch: 'TRA', transfer_receipt: 'REC', physical_count: 'INV' }` + `RESERVED_FOLIO_PREFIXES = ['VTA']` (F4) con **test de contrato** que falla si el enum gana un tipo sin prefijo o si dos tipos comparten prefijo (patrón `UNITS` vs tabla `units`); `documents.service.ts#create(tx, { tenantId, type, warehouseId, linkedWarehouseId?, reasonCode, reference?, reasonNote?, authorizedBy?, transferId?, lineCount, createdBy })` que llama a `nextFolio(tx, tenantId, type, FOLIO_PREFIXES[type])` y devuelve `{ id, folio, type }`, **siempre dentro de la transacción del ledger**; docblock con la regla de oro (el folio se toma en el commit, jamás en la previa) y con el gotcha del lock por `(tenant, key)`
  - **Verificar:** unit del contrato de prefijos (los 5 tipos, sin repetidos, `VTA` no asignada); integración: `create` dentro de una tx que hace rollback no avanza la serie; dos tipos distintos avanzan series independientes; el documento nace con el `line_count` real
  - **Depende de:** F3-DOC-01, F3-DB-03
  - **Estimación:** 2.5 h

- [ ] **F3-DOC-03** — `GET /inventory/documents` y `GET /inventory/documents/:id`
  - **Salida:** `DocumentsController` (`inventory:read`, módulo inventory): filtros `type`, `warehouseId` (∈ scope, si no 403), `from`, `to`, `createdBy`, **`folio`** (búsqueda parcial, case-insensitive — se busca por el número que trae el papel en la mano); `page/pageSize`; solo documentos cuyo `warehouse_id` ∈ scope; orden total `created_at DESC, folio DESC`; fila `{ id, folio, type, warehouse: {id,name}, linkedWarehouse | null, reasonCode, reference, lineCount, createdAt, createdBy: {id,name} }`. Detalle: la cabecera completa + `lines: [{ productId, sku, name, baseUnit, presentation: {id,name,factor} | null, quantityInput, quantityBase, unitCost, lot: {lotCode, expiresAt} | null, location, parentProduct | null }]` leídas de `stock_movements` por `document_id` (orden `seq`); 404 si el almacén no está en scope
  - **Verificar:** `inventory-documents.e2e-spec.ts`: tras una entrada y una salida el listado trae dos filas con folios `ENT-000001` y `SAL-000001`; `folio=ENT` filtra solo entradas; `folio=000001` encuentra las dos; Manager con scope [A] no ve el documento de B ni por listado ni por id (404); las líneas del detalle salen en el orden en que se capturaron; cross-tenant 404
  - **Depende de:** F3-DOC-02, F3-CORE-03
  - **Estimación:** 2.5 h

- [ ] **F3-DOC-04** — Render PDF + `GET /inventory/documents/:id/pdf`
  - **Salida:** dependencia `pdfmake` (server-side; elegido sobre `pdfkit` porque **pagina la tabla solo y repite el encabezado**, y un conteo son 500 líneas); `document-pdf.renderer.ts` con encabezado y pie **comunes** y **cuerpo por tipo**: cabecera con `tenant.legalName ?? tenant.name` + `tenant.taxId`, el tipo en grande y el **folio**; bloque de datos (almacén — y destino si es traspaso —, fecha, quién registró, motivo, referencia, nota, autorizó); tabla `entry`/`exit`/`transfer_*`: `# · SKU · Producto · Presentación · Cantidad (con equivalencia en base_unit) · Lote/Caducidad/Ubicación si aplica · Costo unitario solo en `invoice``; tabla `physical_count`: `# · SKU · Producto · Teórico · Contado · Diferencia`; pie con **total de LÍNEAS** (nunca un total de unidades: sumar 36 unidades + 2.5 kg no significa nada) y tres líneas de firma **Entregó / Recibió / Autorizó**; labels por `nestjs-i18n` con el locale del usuario; endpoint (`inventory:read`, mismo scope que el detalle) → `application/pdf` + `Content-Disposition: attachment; filename="ENT-000001.pdf"`
  - **Verificar:** `documents-pdf.e2e-spec.ts`: 200 con `content-type: application/pdf`, el buffer arranca con `%PDF-` y el `filename` es el folio; un documento de 300 líneas produce **más de una página** (se cuenta `/Type /Page`); en `en` los labels salen en inglés; fuera de scope 404. Unit del renderer: el `docDefinition` de un `physical_count` trae las columnas teórico/contado/diferencia y el de una entrada no; **ningún tipo emite un total de unidades**
  - **Depende de:** F3-DOC-03
  - **Estimación:** 3.5 h

- [ ] **F3-DOC-05** — UI `/movements/documents`: listado y búsqueda por folio
  - **Salida:** ruta con `PermissionGate need="inventory:read"`; buscador por folio con debounce; filtros tipo (multi), `WarehouseSelect` scoped, rango de fechas y usuario; tabla folio (mono, link al detalle), badge de tipo con color por tipo, almacén, fecha, líneas, quién; paginación server-side; estado vacío distinto para "sin documentos" y "sin resultados para este filtro"; `lib/inventory/documents-api.ts` + hooks
  - **Verificar:** `routes/movements-documents.test.tsx` (routeTree real): escribir en el buscador manda `folio` en el request; cambiar el filtro de tipo vuelve a página 1; el badge sale del `type` (dato, no copy); sin permiso → panel del gate
  - **Depende de:** F3-DOC-03, F3-NAV-02
  - **Estimación:** 3 h

- [ ] **F3-DOC-06** — UI detalle del documento + `DownloadDocumentButton`
  - **Salida:** `/movements/documents/$documentId`: cabecera (folio grande, tipo, almacén, fecha, motivo, referencia, nota, quién registró y quién autorizó), tabla de líneas con la equivalencia en base_unit y las columnas de lote solo si alguna línea las trae, link al traspaso si lo hay; `components/inventory/download-document-button.tsx` reusable que pide el PDF con axios `responseType: 'blob'` y dispara la descarga (mismo helper que `downloadImportTemplate`: un `<a href>` iría sin el Bearer y daría 401), con estado de carga y error; se usa en el detalle, en el panel de éxito de **cada** movimiento y en la fila del kardex
  - **Verificar:** test web: el botón pide el blob al endpoint del documento y nombra el archivo con el folio; el detalle de un conteo muestra teórico/contado; un documento sin lotes no renderiza esas columnas
  - **Depende de:** F3-DOC-04, F3-DOC-05
  - **Estimación:** 2 h
...
- [ ] **F3-CORE-07** — Audit por lote de movimientos
  - **Salida:** `movement-audit.ts#recordMovementAudit(tx, auditService, { user, action, documentId, folio, warehouseId, reasonCode, lines: [{ productId, quantity, balanceAfter, parentProductId? }], extra? })` con `resourceType: 'inventory_document'`, `resourceId: documentId` y el **folio** en el payload (es lo que una persona busca cuando audita); acciones canónicas `inventory.entry`, `inventory.exit`, `inventory.transfer_dispatch`, `inventory.transfer_receive`, `inventory.transfer_cancel`, `inventory.physical_count_approve`
...
- [ ] **F3-ENTRY-01** — `POST /inventory/entries`
  - **Salida:** `InventoryController` (`inventory:movement`): body `{ warehouseId, reasonCode, reasonNote?, reference?, authorizedBy?, transferId?, lines: [{ productId, presentationId?, quantity, unitCost?, lotCode?, expiresAt?, location? }] }`; motivos aceptados: `DIRECT_ENTRY_REASONS` + `transfer` (solo con `transferId`, delega en F3-TRANSFER-03); `sale_return`/`physical_count` → 422 `inventory.reason_not_allowed`; scope (403) + almacén activo (422); `authorizedBy` debe ser usuario del tenant (422 `inventory.authorizer_invalid`); `withTenantContext` → `resolveLines` → **`documents.create('entry')`** (folio `ENT`, dentro de la misma tx) → `ledger.apply` → `recordMovementAudit('inventory.entry')`; 201 `{ document: { id, folio, type }, movements: [{ id, productId, quantityBase, presentationId }], stock: [...] }`
  - **Verificar:** `inventory-entries.e2e-spec.ts`: entrada `invoice` con presentación ×12 y 3 cajas → saldo 36 y `unit_cost` guardado; `invoice` sin `unitCost` → 400 en `lines.0.unitCost`; presentación solo-enteros con `1.5` → 422 nombrando la presentación; compuesto → 409; producto con `tracks_lots` sin `lotCode` → 422 `inventory.lot_required`; entrada con lote nuevo crea el `ProductLot` y la fila `(lot, warehouse, location)`; la respuesta trae `document.folio` con formato `ENT-000001` y la segunda entrada del tenant `ENT-000002`; los movimientos creados comparten `document_id`; una entrada que falla por stock **no** consume folio (la serie sigue en el número anterior); Manager fuera de scope → 403; Viewer → 403 por permiso; cross-tenant 404
  - **Depende de:** F3-CORE-03, F3-CORE-05, F3-CORE-07, F3-DOC-02
  - **Estimación:** 3 h
...
- [ ] **F3-ENTRY-03** — UI tabla de líneas + selector de presentación + UX teclado
  - **Salida:** `components/inventory/movement-lines-table.tsx` (reusable por salida): buscador con autocompletado server-side (`GET /products?query`, patrón del picker de BOM) que agrega fila con foco en Cantidad; por línea: SKU, nombre, **selector de presentación** (comprables activas; preselecciona la comprable por defecto o la base), cantidad (`step` 1 o `0.0001` según `allow_fractional_input`, error en vivo de escala), equivalencia en base_unit ("3 Caja = 36 unidad" con `unitName`), columna Costo unitario (`MONEY_STEP`) solo con `invoice` + total calculado, quitar; **lote**: si el producto tiene `tracks_lots`, la fila muestra `Lote` (texto con autocompletado de lotes existentes del producto vía `GET /products/:id/lots`), `Caducidad` (date; se autocompleta si el lote existe y queda solo-lectura) y `Ubicación` (texto, autocompletado de ubicaciones ya usadas en el almacén); sin `tracks_lots` esas columnas no se renderizan; **teclado**: Enter en cantidad → vuelve al buscador, Esc en el buscador → limpia, `Ctrl/⌘+Enter` → confirmar; producto ya presente en la tabla → suma foco a esa fila en vez de duplicar (misma presentación) o agrega fila nueva (otra presentación)
  - **Verificar:** test del componente: agregar producto con 2 presentaciones muestra el selector con la default marcada; cambiar presentación recalcula la equivalencia; presentación solo-enteros con `1.5` marca error antes de enviar; producto con `tracks_lots` muestra las 3 columnas de lote y sin él no; secuencia de teclado devuelve el foco al buscador
  - **Depende de:** F3-ENTRY-02, F3-LOTS-02
  - **Estimación:** 3.5 h
...
- [ ] **F3-ENTRY-05** — Plantilla y vista previa de entrada (`template` + `preview`)
  - **Salida:** **muda `spreadsheet.ts`** de `modules/products/` a `common/spreadsheet/` (la usan conteo, entradas y salidas; deja de ser detalle de un módulo) y actualiza los imports de productos; `GET /inventory/entries/template?format=xlsx|csv` (`inventory:movement`) con columnas `sku, presentacion, cantidad, costo_unitario, lote, caducidad, ubicacion` y una fila de ejemplo; **`POST /inventory/entries/preview`** (`inventory:movement`): acepta **o** `lines[]` **o** `file` + `format` — la misma respuesta en los dos casos, que es lo que permite que la pantalla sea una sola; > 5 MB → 413; archivo ilegible → 400; corre **exactamente** las mismas validaciones que `POST /inventory/entries` (`resolveLines`, motivo, scope, almacén activo, presentación, escala, lotes) pero **no escribe nada y no pide folio**; lee el saldo del momento y devuelve `{ warehouse, reasonCode, rows: [{ lineIndex, productId, sku, name, baseUnit, presentation, quantityInput, quantityBase, unitCost, lotCode?, expiresAt?, location?, newLot?, stockBefore, stockAfter }], summary: { lines, products, newLots }, errors: [{ row, field, message, code }] }`
  - **Verificar:** `inventory-entries-preview.e2e-spec.ts`: la previa de 3 cajas ×12 sobre un saldo de 10 devuelve `stockBefore: 10` y `stockAfter: 46` **y el saldo real sigue en 10**; ningún `stock_movement`, `inventory_document` ni `product_lot` creado; **la serie de folios no avanzó** (la previa no quema números); un Excel y unas `lines[]` equivalentes producen la MISMA respuesta; archivo con un sku inexistente y una escala inválida devuelve los dos errores con su `row`; compuesto → aparece como error de fila, no revienta la previa
  - **Depende de:** F3-ENTRY-01, F3-COUNT-01
  - **Estimación:** 3 h

- [ ] **F3-ENTRY-06** — UI: carga por Excel y paso de vista previa
  - **Salida:** la pantalla de entrada pasa a **dos pasos**. Paso 1 (captura): la tabla manual de F3-ENTRY-03 **más** "Descargar plantilla .xlsx / .csv" y un control de archivo con forma de botón (input `sr-only` + label, patrón F2-IMPORT-05) que reemplaza/añade las líneas cargadas; el botón principal deja de ser "Confirmar" y pasa a ser **"Ver vista previa"** → `POST …/preview`. Paso 2 (previa): tabla de solo lectura con SKU, producto, presentación, cantidad, **stock actual → stock resultante** (la columna que pidió Carlos), lote/caducidad/ubicación si aplican, y las filas con error marcadas con su mensaje; resumen arriba (líneas, productos, lotes nuevos, errores); "Volver a editar" y **"Confirmar entrada"** (deshabilitado si hay errores); confirmar manda las **mismas líneas** ya validadas a `POST /inventory/entries`
  - **Verificar:** test web: cargar un archivo llama a `preview` y NO a `entries`; la tabla de previa muestra `stockBefore → stockAfter` por fila; con un error de fila el botón de confirmar está deshabilitado y el mensaje sale sobre la fila; "Volver a editar" conserva las líneas; confirmar manda a `entries` lo mismo que se previsualizó
  - **Depende de:** F3-ENTRY-05, F3-ENTRY-04
  - **Estimación:** 3 h
...
- [ ] **F3-EXIT-01** — `POST /inventory/exits` (incluye despacho de traspaso)
  - **Salida:** (`inventory:movement`) body `{ warehouseId, reasonCode, reasonNote?, reference?, authorizedBy?, linkedWarehouseId?, lines[] }`; motivos `DIRECT_EXIT_REASONS`; `sale`/`physical_count` → 422 `inventory.reason_not_allowed`; scope + almacén activo; compuestos: `consumption|expired` → `expandComposition` y salidas de componentes en la misma tx, otros motivos → 409; **`transfer`**: `linkedWarehouseId` obligatorio, activo, del tenant, ≠ origen (422 `inventory.transfer_same_warehouse`), **sin exigir scope sobre el destino**; en la tx: `documents.create('transfer_dispatch')` (folio `TRA`), `Transfer { status: in_transit, dispatchDocumentId }` + `TransferLine` (agrupadas por producto en base_unit, `quantity_sent`), movimientos con `transfer_id` y `linked_warehouse_id`; una salida directa crea en cambio un documento `exit` (folio `SAL`); audit `inventory.exit` o `inventory.transfer_dispatch`; 201 `{ document: { id, folio, type }, movements, stock, transfer?: { id } }`
  - **Verificar:** `inventory-exits.e2e-spec.ts`: salida > stock → 422 `{ sku, available, requested }` y saldo intacto; salida de producto con lote sin `lotId` → FEFO (el ejemplo de Carlos: baja st10 a 9); con `lotId` forzado → ese lote; `transfer` de producto con lote → `transfer_lines` con `lot_id` y la recepción entra al **mismo lote** en el destino; `consumption` de compuesto descuenta componentes con `parent_product_id` y saldo del compuesto no existe; `loss` de compuesto → 409; una salida directa devuelve `SAL-000001`; `transfer` crea `Transfer` `in_transit` cuyo documento de despacho es `TRA-000001`, baja el origen y NO toca el destino; segundo traspaso del tenant → `TRA-000002` mientras la serie `SAL` sigue su propia cuenta; `transfer` a sí mismo → 422; Manager con scope [A] despacha de A hacia B (fuera de su scope) → 201
  - **Depende de:** F3-CORE-03, F3-CORE-05, F3-CORE-06, F3-CORE-07, F3-CORE-08, F3-DB-03, F3-DOC-02
  - **Estimación:** 3.5 h
...
- [ ] **F3-EXIT-03** — UI líneas con disponible en vivo
  - **Salida:** `movement-lines-table.tsx` en `mode='exit'`: al agregar un producto consulta `GET /products/:id/stock` y muestra **Disponible** en el almacén elegido (en base_unit y en la presentación elegida: "120 unidad = 10 Caja"); cantidad que excede el disponible (sumando líneas del mismo producto) → error en vivo y confirmar deshabilitado; cambiar el almacén origen refresca disponibles; compuesto → fila con leyenda "se descontarán sus componentes" + unidades armables (`GET /products/:id/availability`) como techo; producto con `tracks_lots` → debajo de la fila un desglose **por lote** (lote, caducidad, ubicación, disponible; el que FEFO tomaría marcado "se descuenta primero") y un selector opcional "forzar lote" que manda `lotId`
  - **Verificar:** test del componente: disponible 10, cantidad 11 → error y botón deshabilitado; producto con lotes muestra el desglose con el primero marcado; dos filas del mismo producto 6 + 5 → error; compuesto muestra el techo por unidades armables
  - **Depende de:** F3-EXIT-02, F3-ENTRY-03, F3-KARDEX-03, F3-LOTS-02
  - **Estimación:** 3.5 h
...
- [ ] **F3-EXIT-05** — Plantilla y vista previa de salida
  - **Salida:** `GET /inventory/exits/template` (columnas `sku, presentacion, cantidad, lote, ubicacion` — sin costo: una salida no tiene precio de compra) y **`POST /inventory/exits/preview`**, mismo contrato y mismas dos formas de carga que la entrada, reusando el servicio de previa; además por línea: **`available`** en el almacén y, para los productos con lote, **el reparto FEFO que se aplicaría** (`fefoPlan: [{ lotId, lotCode, expiresAt, location, quantity }]`) — así el usuario ve de qué lote va a salir **antes** de confirmar; `stockAfter` puede dar negativo → la fila se marca con error `insufficient_stock` en vez de reventar la previa entera
  - **Verificar:** `inventory-exits-preview.e2e-spec.ts`: el ejemplo de Carlos (lotes st30/st10/st60, salida de 1) devuelve `fefoPlan` con st10 y `stockAfter` 9, **sin mover el stock**; pedir más de lo disponible marca esa fila con `available` y `requested` y deja las demás válidas; un compuesto con `consumption` muestra la expansión de componentes en la previa; la serie de folios no avanza
  - **Depende de:** F3-EXIT-01, F3-ENTRY-05, F3-CORE-08
  - **Estimación:** 2.5 h

- [ ] **F3-EXIT-06** — UI de dos pasos para salidas
  - **Salida:** mismo flujo que F3-ENTRY-06 sobre la pantalla de salida (plantilla, carga por Excel, "Ver vista previa" → previa → "Confirmar salida"); la tabla de previa suma **Disponible** y, en los productos con lote, una sub-fila con el **reparto FEFO** ("saldrá 1 del lote st10, vence 01/07/2026"); las filas con `insufficient_stock` en rojo con el disponible real; éxito → folio `SAL` (o `TRA` si fue traspaso) + botón de PDF
  - **Verificar:** test web: la previa muestra el reparto FEFO por línea; una fila sin stock suficiente bloquea el confirmar y las otras siguen visibles; el éxito muestra el folio y el botón de descarga
  - **Depende de:** F3-EXIT-05, F3-EXIT-04, F3-ENTRY-06
  - **Estimación:** 2.5 h
...
> Tres pasos stateless: plantilla del almacén → subir contado → reconciliar (puro) → aprobar (tx con `FOR UPDATE`, sin bloqueo previo del almacén). Solo `inventory:manage` aprueba. **Una sola plantilla para todo**: los productos con `tracks_lots` ocupan una fila por (lote, ubicación) con `lot_code`/`expires_at`/`location`; los demás una fila con esas columnas vacías — es exactamente el Excel del cliente de Carlos.
...
- [ ] **F3-COUNT-02** — `POST /inventory/counts/reconcile` (dry-run puro)
  - **Salida:** (`inventory:movement`) body `{ warehouseId, file (base64), format }`; > 5 MB → 413 `inventory.count_file_too_large`; archivo ilegible → 400; por fila: `sku` existe, activo, no compuesto; `counted` numérico ≥ 0 con ≤ 4 decimales, **entero si la presentación base (factor 1) es solo-enteros**; **lotes**: si el producto tiene `tracks_lots`, `lot_code` es obligatorio (error de fila) y la clave de la fila es `(sku, lot_code, location)`; un `lot_code` que no existe se marca `newLot: true` (se creará al aprobar; `expires_at` obligatoria si el producto lo exige) y si existe con otra `expires_at` → error de fila; si el producto NO tiene `tracks_lots` y la fila trae lote → error de fila; duplicado de la clave en el archivo → error de fila; `counted` vacío → fila `skipped`; **no escribe nada**; responde `{ rows: [{ productId, sku, name, unit, lotCode?, expiresAt?, location?, newLot?, theoretical, counted, difference }], summary: { counted, matches, discrepancies, skipped, newLots }, errors: [{ row, field, message }] }` (teórico leído en ese momento)
...
- [ ] **F3-COUNT-03** — `POST /inventory/counts/approve` (transacción)
  - **Salida:** (`inventory:manage`) body `{ warehouseId, lines: [{ productId, lotCode?, expiresAt?, location?, counted, theoreticalSeen }] }` (el resultado reconciliado, NO el archivo); scope; en UNA tx: crea los `ProductLot` nuevos, `FOR UPDATE` ordenado sobre `stock_by_warehouse` **y** `stock_lots`, relee el teórico **fresco** (por lote cuando aplica); por línea con `counted ≠ theoreticalNow`: **salida `physical_count` del teórico total** (si > 0) + **entrada `physical_count` del contado** (si > 0), mismo `document_id`, con `lot_id`/`location` cuando aplica (así la invariante lote/total se sostiene: el ledger las mueve juntas); iguales → sin movimiento (el CHECK `quantity > 0` lo exige); todo lo generado cuelga de **un** documento `physical_count` (folio `INV`) creado en la misma tx; audit `inventory.physical_count_approve` con `summary` y por línea `{ productId, theoreticalSeen, theoreticalAtApproval, counted, drifted: seen ≠ now }`; 201 `{ document: { id, folio, type }, applied, unchanged, drifted }`
  ...
  - **Depende de:** F3-COUNT-02, F3-CORE-05, F3-CORE-07, F3-DB-07, F3-DOC-02
  - **Estimación:** 3.5 h

- [ ] **F3-COUNT-04** — UI `/movements/inventory-count` paso 1
  - **Salida:** ruta con gate `inventory:movement`; `WarehouseSelect` scoped; botones "Descargar plantilla .xlsx / .csv"; control de archivo como botón (input `sr-only` + label, patrón F2-IMPORT-05); "Procesar" → `reconcile`; errores por fila en tabla con descarga csv; sin checkbox de bloqueo (decisión) y leyenda "el conteo se aplica sobre el saldo del momento de aprobar"; leyenda de la plantilla: "los productos con lote traen una fila por lote y ubicación; deja las columnas de lote vacías en los que no lo manejan"
...
- [ ] **F3-LOTS-03** — Alertas de vencimiento (chicas, sin jobs)
  ...
  - **Depende de:** F3-LOTS-02, F3-EXIT-04, F3-NAV-02
  - **Estimación:** 3 h
...
- [ ] **Todo movimiento es un documento con folio**: una entrada da `ENT-000001`, una salida `SAL-000001`, un despacho `TRA-000001`, su recepción `REC-000001` y un conteo `INV-000001`; las cinco series avanzan **independientes** por tenant, dos tenants arrancan las suyas en 1, y una operación que falla **no deja hueco** en la serie
- [ ] **Vista previa antes de tocar el stock**: una entrada de 80 productos se carga por Excel (o a mano), se ve en una tabla con **stock actual → stock resultante** por línea, con los errores marcados sobre su fila, y recién entonces se confirma; la previa **no escribe nada ni consume folio** (verificado: la serie sigue en el mismo número); en salidas la previa muestra además el disponible y **de qué lote saldría por FEFO**
- [ ] **PDF firmable**: cualquier documento se baja en PDF con el negocio, el folio, sus líneas y las firmas Entregó / Recibió / Autorizó; uno de 300 líneas sale paginado con el encabezado repetido; se encuentra después por folio desde `/movements/documents` y se vuelve a bajar
- [ ] `stock_movements` **e `inventory_documents`** son append-only por privilegios (UPDATE/DELETE como `sellpoint_app` fallan) y las 5 tablas nuevas pasan los 4 canarios RLS
...
**Estimación: 5-6 semanas** (~140 h en 62 tareas). El outline original decía 2-3 semanas y no contaba kardex con saldo, stock por almacén, el conteo completo ni las guardas heredadas; después entraron los **lotes con FEFO** (2026-08-17, sobre un Excel real de cliente) y los **folios, la vista previa y el PDF** (2026-08-18, revisión de Carlos antes de seguir construyendo: +10 tareas, ~27 h). Las tres veces el alcance creció por evidencia concreta, no por especulación.
